...
export class ReorderWorkoutComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private initSortable(): void {
    ...
    const changeIndex = (item: Sortable, to: number) => {
      ...
      console.log(
        "Ordine aggiornato:",
        this.sortables.map((s) => s.workoutIdentifier),
      );
    };
    ...
    cardRows.forEach((cardRow, index) => {
      ...
      const workoutIdentifier =
        this.workouts[index]?.form.get("identifier")?.value ?? -1;
  ...
  getControl(
    allenamentoForm: AllenamentoForm,
    controlName: string,
  ): FormControl {
